...
public class TestQueryHBaseRdfFactory extends TestFactoryManifest
{
	...
	{
		TestQueryHBaseRdfFactory f = new TestQueryHBaseRdfFactory( storeDesc, testRootName ) ;
	...
	private TestQueryHBaseRdfFactory( StoreDesc storeDesc, String testRootName )
	{
	...
	public Test makeTest( Resource manifest, Resource entry, String testName, Resource action, Resource result )
	{
		...
		if ( testItem.getTestType() != null )
		{
			if ( testItem.getTestType().equals( TestManifestX.TestQuery ) )
				test = new TestQueryHBaseRdf( storeDesc, testName, results, testItem ) ;
