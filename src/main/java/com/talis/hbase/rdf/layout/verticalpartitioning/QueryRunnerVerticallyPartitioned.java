...
public class QueryRunnerVerticallyPartitioned extends TableQueryRunnerBase implements TableQueryRunnerBasics
{
	public QueryRunnerVerticallyPartitioned( String storeName, HBaseRdfConnection connection ) { super( storeName, connection ) ; }
		...
		try
		{
			if( tblType.equalsIgnoreCase( "sub" ) )
			{
				...
				if( tblType.equalsIgnoreCase( "obj" ) )
				{
					...
					if( pm.isConcrete() ) 
					{
						HTable table = tables().get( name() + "-" + tblPrefix + "-" + HBaseUtils.getNameOfNode( pm ) + "-objects" ) ;
						Result rr = null ; if( table != null ) rr = table.get( res ) ;
						if( rr != null && !rr.isEmpty() ) trIter = new HBaseRdfSingleRowIterator( rr, sm, pm, om, pm.toString(), TableDescVPCommon.COL_FAMILY_NAME_STR ) ;
					}
					else
