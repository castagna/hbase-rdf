...
public class QueryRunnerHybrid extends TableQueryRunnerBase implements TableQueryRunnerBasics
{
	public QueryRunnerHybrid( String storeName, HBaseRdfConnection connection ) { super( storeName, connection ) ; }
		...
		try
		{
			...
			if( tblType.equalsIgnoreCase( "sub" ) )
			{
				...
				if( pm.isConcrete() ) 
				{
					...
					if( rr != null && !rr.isEmpty() ) trIter = new HBaseRdfSingleRowIterator( rr, sm, pm, om, pm.toString(), TableDescHybridCommon.COL_FAMILY_NAME_STR ) ;
				}
				else
				{
					...
					if( rr != null && !rr.isEmpty() ) trIter = new HBaseRdfSorOSingleRowIterator( rr, sm, pm, om, TableDescHybridCommon.COL_FAMILY_NAME_STR ) ;
				}
				res = null ;
			}
			else 
				if( tblType.equalsIgnoreCase( "obj" ) )
				{
					...
					if( tblType.equalsIgnoreCase( "pred" ) )
					{
						...
						HTable table = tables().get( sb.toString() ) ;
						if( table != null ) trIter = new HBaseRdfSingleTableIterator( table.getScanner( scanner ), sm, pm, om, pm.toString(), TableDescHybridCommon.COL_FAMILY_NAME_STR ) ;
					}
					else
						if( tblType.equalsIgnoreCase( "all" ) )
						{
							...
							sb.append( TableDescSubjects.SUBJECT_TBL_NAME ) ;
							HTable table = tables().get( sb.toString() ) ;
							if( table != null ) trIter = new HBaseRdfSorOSingleTableIterator( table.getScanner( scanner ), sm, pm, om, TableDescHybridCommon.COL_FAMILY_NAME_STR ) ;
						}
			sb = null ;
		}
