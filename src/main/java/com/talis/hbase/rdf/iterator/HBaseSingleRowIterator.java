 ...
 */
public class HBaseSingleRowIterator extends AbstractIterator<Triple> {
	 ...
	 */
	public HBaseSingleRowIterator( Result rr, Node sm, Node pm, Node om )
	{
		this.subject = sm; this.predicate = pm; this.object = om;
		this.row = Bytes.toString( rr.getRow() );
		resultTriples = Bytes.toString( rr.getValue( HBaseUtils.COL_FAMILY_NAME_BYTES, HBaseUtils.COL_QUALIFIER_NAME_BYTES ) ).split( "&&" );
	}
